 ...
 *	GT_FAIL on fail
 */
GT_STATUS rm_parse_header(RMU_FRAME_HEADER *header, unsigned char* buf, int len)
{
	unsigned int idx = 0;
 ...
 *		RM_BADMSG on fail
 */
GT_STATUS rm_parse_data(unsigned char* usrbuf, RMU_FRAME_HEADER *header)
{
	unsigned int len = header->data_len;
	GT_STATUS status;

	DBG_INFO((RM_DBG_FRAME, "frame: rm_parse_data called\n"));
...
GT_STATUS rm_res_handler(RMU_FRAME_HEADER *req_header, unsigned char* buf, int len )
{
