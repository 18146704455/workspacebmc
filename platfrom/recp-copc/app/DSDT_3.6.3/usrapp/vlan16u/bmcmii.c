 ...
 * @version: 
 * @Author: smith.zj
 * @Date: 2019-10-17 13:15:46
 * @LastEditors: smith.zj
 * @LastEditTime: 2020-05-06 15:28:06
 */
#include <Copyright.h>
#include <msApi.h>
...
out:
    ...
    int i=0;
    value = *(volatile uint32_t *)(base+MDIO_CTL_OFF);
    value |= (regaddr & 0x1f);      /*reg*/
    value |= (phyaddr & 0x1f) << 5; /*phyaddr*/
    value |= 3 << 10;
    value |= 1 << 15;
...
#define MAC_PHYBUSY_NEW           0x00008000   
    ...
    *(volatile uint32_t *)((char*)base + MDIO_CTL_OFF) = tmp;

    do{  /*wait for compl*/
        if(!( (*(volatile uint32_t *)((char*)base + MDIO_CTL_OFF)) & MAC_PHYBUSY_NEW )) {
            break;
        }
        usleep(100);
    }while(i++ < RETRY_TIMES);
...
int phy_read(char busno, char phyaddr, char regaddr, uint16_t* val)
{
    char *base;
    int ret = 0;
    uint32_t value = 0;
    ...
    value = *(volatile uint32_t *)((char*)base + 0x40);
    if(((value) >>31)&0x1 )
    {
...
int phy_write(char busno, char phyaddr, char regaddr, uint16_t val)
{
    char *base=NULL;
    int ret = 0;
    volatile uint32_t *value;
...
GT_BOOL gtBspReadMii(GT_QD_DEV* dev, unsigned int portNumber , unsigned int MIIReg,unsigned int* value)
{
    ...
    iret = phy_read(SWITCH_MII_BUS, portNumber, MIIReg, (uint16_t*)value);
  //  DB_PRINT(DB_DEBUG,"deviceId 0x%x protNumber 0x%x MIIReg 0x%x value 0x%x iret %d",dev->deviceId,portNumber,MIIReg,*value,iret);
    if(iret != 0) {
        DB_PRINT(DB_ERR,"read port 0x%x reg 0x%x failed",portNumber,MIIReg);
        bret = GT_FALSE;
    }
...
GT_BOOL gtBspWriteMii(GT_QD_DEV* dev, unsigned int portNumber , unsigned int MIIReg,unsigned int value)
{
    ...
    GT_BOOL bret = GT_TRUE;
 //   DB_PRINT(DB_DEBUG,"deviceId 0x%x protNumber 0x%x MIIReg 0x%x value 0x%x iret %d",dev->deviceId,portNumber,MIIReg,value,iret);
    iret = phy_write(SWITCH_MII_BUS, portNumber, MIIReg, (uint16_t) value);
    if(iret != 0) {
        DB_PRINT(DB_ERR,"write port 0x%x reg 0x%x failed",portNumber, MIIReg);
        bret = GT_FALSE;
    }
