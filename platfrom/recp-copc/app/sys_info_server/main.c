...
{  
    ...
    pr_info("start init_bsp_info\n");
    if(argc >= 2) {
        ret = init_bsp_info(argv[1]);
    } else {
        ret = init_bsp_info(NULL);
    }
    
    if(ret != 0) {
        pr_err("init_bsp_info failed\n");
        return -1;
    }
    ...
    {  
        pr_err("setsockopt error in reuseaddr[%d]\n", sd);  
        return -1;  
    }  
...
{  
    ...
    */  
    ...
    if(w_client == NULL)  
    {  
        pr_err("malloc error in accept_cb\n");  
        return ;  
    }  
      
    if(EV_ERROR & revents)  
    {  
        pr_err("error event in accept\n");  
        return ;  
    }  
    ...
    {  
        pr_err("client_sd not NULL fd is [%d]\n", client_sd);  
        return ;  
    }  
...
{  
    ...
    {  
        //buffer[read] = '\0'; 
        deal_rpc_request(watcher->fd, buffer, read); 
        DB_PRINT(DB_DEBUG, "receive message:%s\n", buffer);  
    }  
    //返回给客户端  
    bzero(buffer, read);  
}  
